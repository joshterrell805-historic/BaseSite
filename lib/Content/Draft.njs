...
Draft.create = function create(doc) {
  ...
  return mysqlQuery('INSERT INTO drafts (doc_id, title, body, hash, doctype) ' +
      'VALUES (?, ?, ?, ?, ?)',
      [doc.id || null, doc.title, doc.body, hash, doc.doctype])
  .then(function(results) {
    assert(results.affectedRows === 1);
    return mysqlQuery('SELECT id FROM drafts WHERE title=?', [doc.title])
    .then(function(results) {
...
Draft.getIndex = function getIndex(session) {
  if (!session)
    return Promise.resolve([]);
  return mysqlQuery(
      'SELECT title, id, edit_ts FROM drafts ORDER BY edit_ts DESC')
  .then(function (results) {
    return results.map(function(result) {
      return {
        title: result.title,
        url: '/drafts/' + result.id,
